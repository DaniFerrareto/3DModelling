...
//////////////////////////////////////////////////
module 3d_array_sp_cb(x,y,z,r,d,h,nx,ny,nz,geo,cbORsp,res,nome="Conjunto 1"){
    //----- FUNÇÃO ARRAY 3D DE ESFERAS OU CUBOS -----//
    /*
    Forneça:
        Dimensões dos cubos: x,y,z
        Raio das esferas: r
        Distância entre os centros de cada esfera/cubo: d
        Quantidade de cilíndros nas três direções: nx,ny,nz
        Resolução das esferas: res
    Selecione a geometria:
        Retangular: geo=1
        Hexagonal: geo=2
    Selecione o objeto:
        cbORsp="cube"
        cbORsp="sphere"
    Dê um nome para seu conjunto de esferas/cubos: nome
    */
    if (d<2*r){
        text("Error!");
        translate([0,-15,0])text("Verify your specified dimensions.");
        ...
        if(geo==1){
            for(k=[0:1:nz-1]){
                for(j=[0:1:ny-1]){
                    for(i=[0:1:nx-1]){
                        if(cbORsp=="cube"){
                            translate([i*d,j*d,k*d])cube([x,y,z]);
                        }else if(cbORsp=="sphere"){
                            translate([i*d,j*d,k*d])sphere(r,$fn=res);
                        }else{
                            text("Error!");
...
3d_array_sp_cb(1,1,1,1,5,30,5,5,5,1,"sphere",10,"Conjunto 1");