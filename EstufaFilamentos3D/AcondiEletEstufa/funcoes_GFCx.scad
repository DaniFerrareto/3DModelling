...
//////////////////////////////////////////////////
module aux3DarrayObjects(x,y,z,res,r,h,r1,r2,d,objeto,translatex,translatey,translatez){
    /*
    Objetos: Cubos, Esferas, Cilíndros etc.
    Forneça:
        As dimensões dos objetos:
            Cubos (Paralelepípedos): x,y,z
            Esferas: r
            Cilíndros (Cones e Troncos de Cones): h,r1,r2
        Distância entre os centros de cada objeto: d
        Resolução de objetos curvos: res (Recomendável: res<20)
    Selecione o objeto:
        objeto="cube"
        objeto="sphere"
        objeto="cylinder"
    Deslocamento do objeto é dado por um:
        translate([translatex,translatey,translatez]);
    */
    if(objeto=="cube"){
        translate([translatex,translatey,translatez])cube([x,y,z]);
        centerx=translatex+x/2;
        centery=translatey+y/2;
        centerz=translatez+z/2;
        echo("centerx=",centerx);
        echo("centery=",centery);
        echo("centerz=",centerz);//Colocar dimensões totais 
    }else if(objeto=="sphere"){
        translate([translatex,translatey,translatez])sphere(r,$fn=res);
        centerx=translatex;
        centery=translatey;
        centerz=translatez;
        echo("centerx=",centerx);
        echo("centery=",centery);
        echo("centerz=",centerz);//Colocar dimensões totais 
    }else if(objeto=="cylinder"){
        translate([translatex,translatey,translatez])cylinder(h,r1,r2,$fn=res);
        centerx=translatex;
        centery=translatey;
        centerz=translatez+h/2;
        echo("centerx=",centerx);
        echo("centery=",centery);
        echo("centerz=",centerz);//Colocar dimensões totais 
    }else{
        text("Error!");
        translate([0,-15,0])text("Verify your object type.");
    }
}

//////////////////////////////////////////////////
module 3d_arrayObjects(x,y,z,res,r,h,r1,r2,d,nx,ny,nz,geo,objeto,nome="Conjunto 1"){
    //----- FUNÇÃO ARRAY 3D DE ESFERAS OU CUBOS -----//
    /*
    Objetos: Cubos, Esferas etc.
    Forneça:
        As dimensões dos objetos:
            Cubos (Paralelepípedos): x,y,z
            Esferas: r
            Cilíndros (Cones e Troncos de Cones): h,r1,r2
        Distância entre os centros de cada objeto: d
        Quantidade de objetos nas três direções: nx,ny,nz
        Resolução de objetos curvos: res (Recomendável: res<20)
    Selecione a geometria:
        Cúbica: geo="Cube"
        Tetraédrica: geo="Thetrahedral"
    Selecione o objeto:
        objeto="cube"
        objeto="sphere"
    Dê um nome para seu conjunto de objetos: nome
    */
    
    // Olhe na aba console
        echo("Nome: ", nome);
        echo();
        echo("Quantidade de Objetos (nx*ny*nz):");
        echo(nx*ny*nz);
        echo();
    ...
    if (d<2*r){ // debugs
        ...
        if(geo=="cube"){
            for(k=[0:1:nz-1]){
                for(j=[0:1:ny-1]){
                    for(i=[0:1:nx-1]){
                        aux3DarrayObjects(x,y,z,res,r,h,r1,r2,d,objeto,i*d,j*d,k*d);
                    }
                }
            }//Colocar dimensões totais 
        }else if (geo=="thetahedral"){ // Note as colunas hexagonais se formando
            for(k=[0:1:nz-1]){
                for(j=[0:1:ny-1]){
                    for(i=[0:1:nx-1]){
                        movex=i*d+((j+k)%2)*(d/2);
                        hTr=sqrt(3/4); // Altura do Triângulo Equilátero de Lado 1
                        movey=j*d*sin(60)+(k%2)*d*(hTr/3);
                        hTe=sqrt(2/3); // Altura do Tetraedro Regular de Lado 1
                        movez=k*d*hTe;
                        aux3DarrayObjects(x,y,z,res,r,h,r1,r2,d,objeto,movex,movey,movez);
                    }
                }
            }//Colocar dimensões totais                
        }
...
module funcao_caixa(a,b,c,d,h,encaixe,simounao,nome="Caixa 1"){
