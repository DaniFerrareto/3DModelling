...
polygon(   [[0,pp],[hh,pp],[r-(h/8),(p/16)],[r-h/8,-p/16],[hh,pm],[0,pm]]   ,   [[0,1,2,3,4,5]]   );
}
//>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
...
rotate([90,0,0])
linear_extrude(height = g, center = true, convexity = 10)
polygon(   [[0,pp],[hh,pp],[r,0],[hh,pm],[0,pm]]   ,   [[0,1,2,3,4]]   );
}
//>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
...
stdpitch=get_std_pitch(diameter);
stdclearance=get_std_clearance(diameter);
thread_for_nut_cuiso_tec(diameter+stdclearance+usrclearance,length,stdpitch,divdelta=stddivs,entry=1);
