...
- Since OpenScad version 2021.01, version 1.1 of this library stopped with error. The necessary modifications have been made in this version 1.2 so that it also works with OpenScad 2021.01.
- Due to these necessary modifications, unfortunately now the preview time has increased slightly. 
- Remember that if necessary you can increase the speed for previewing and rendering using the functions with full parameters and specifying a smaller number of the "divs" parameter. If you prefer, you can also improve the speed at a general level by changing the variable "stddivs" located on line 167, increasing this variable increases the preview and rendering time (and increases the resolution), decreasing the value of this variable decreases the time preview and the resolution is lowered. The default value of 50 is generally adequate and in most cases you don't need to worry about this unless your computer is really slow, or you want to get really large diameter threads. This version continues to be compatible with OpenScad versions prior to 2021.01.
- Some warnings that did not affect operation have been removed.
- Some "echo" that did not affect operation have been removed. 
- This version is completely compatible with your projects made with previous versions, the name of the library "threads-library-by-cuiso-v1.scad" has been kept so you do not have to change anything in your source code, you only have to replace the old file "threads-library-by-cuiso-v1.scad" by the new "threads-library-by-cuiso-v1.scad" and you are ready to work with OpenScad 2021.01.
- Happy designing !!!
...
//For use this library you can put the file "threads-library-by-cuiso-v1.scad" in the same directory of your project and put the line "use <threads-library-by-cuiso-v1.scad>" in your own .scad file
...
This library is focused to provide simple functions with preset parameters by default, and not to be too demanding in CPU consumption neither in previsualizacion nor in rendering

This library has been tested printing on PLA with 0.1 layer. Except for smaller diameter threads layers greater than 0.1 should work well. Diamaters tested between 3mm and 10mm, but you can use any diameter of your choice. 

Shape of the threads are similar (but not equal) to ISO METRIC threads. Some tests have been done using screws and nuts printed with this library in combination with screws and nuts of metal with acceptable results. However, they have not been exhaustive tests.
*/
...
>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
SOME NOTES:
- At first preview(F5) of the design may appear in console a lot of messages:  "PolySet has nonplanar faces. Attempting alternate construction" simply ignore those messages, they are not a problem. In versión 1.2 these messages no longer appear.

- I have no lag problems in preview(F5) mode during the test of this library. If you are suffering lag problems in preview you can try to un-comment the "render()" al lines 286 and 293. You can also try decreasing the value of the stddivs variable on line 169 (50 is the default value).
