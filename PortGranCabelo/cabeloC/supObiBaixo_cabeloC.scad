...
difference(){
    // Furo Obilongo de Baixo
    translate([haste2x-haste3x,-esp,haste1z+haste2z+haste3z-haste4z-compriObilongo+compriObilongo+portaz-3*haste4z-3*20+portaz+10])cube([haste4x,esp,haste4z-portaz+haste4z]);
    // Furo Obilongo de Baixo
    #translate([0,0,basePortaz])hull(){
        #translate([haste2x-haste4x/2,-2*esp,haste1z+haste2z*0.8+haste3z-3*haste4z-3*20+(1)*portaz])
        #rotate([-90,0,0])
        #cylinder(4*esp,largObi/2,largObi/2,$fn=res);
